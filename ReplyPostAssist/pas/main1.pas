...
uses
  ...
  ExtCtrls, XPMan, Registry, ShellApi, SyncObjs, System.Actions, UConstants;
...
type
  ...
  private
    { D閏larations priv閑s }
    FLoading: Boolean;
    FLoadingResource: Boolean;
    function IsMain(const b: ICefBrowser; const f: ICefFrame = nil): Boolean;
  public
    property bLoading:Boolean  read FLoading;
    property bLoadingResource: Boolean read FLoadingResource write FLoadingResource;

    procedure ScrollToBottomEXE;
    procedure GetWebBrowserHTMLEXE;
...
begin
  if crm.Browser <> nil then
    crm.Browser.MainFrame.LoadUrl(TaobaoAddres);
  if crm.Browser <> nil then
    crm.Browser.MainFrame.LoadUrl(TaobaoAddres);
end;
...
procedure TMainForm.ScrollToBottomEXE;
var
  js: string;
begin
  js := 'window.scrollTo(0,document.body.scrollHeight);';    //获得ID为"login_button"并点击
...
begin
  ...
  begin
    edAddress.Text := url;
//    strUr := url;
//    SendMessage(G_APPHandle, WM_USER + 105, WPARAM(PChar(strUr)), 0);
  end;
end;
...
begin
  crm.Browser.MainFrame.ExecuteJavaScript(sJS, 'about:blank', 0);   //执行js
end;
...
begin
  FLoading := False;
  edAddress.Text := TaobaoAddres;
end;

procedure TMainForm.GetWebBrowserHTMLEXE;
begin
  crm.Browser.MainFrame.GetSourceProc(CallbackGetSource);
end;
